...
- **Scratch files go in the session scratchpad or `$TMPDIR`, not bare `/tmp`.** Use the scratchpad directory named in the system prompt, or `D="$TMPDIR/<name>"; mkdir -p "$D"`. The harness itself instructs this — `$TMPDIR` is set to the correct sandbox-writable directory — and a session-scoped directory keeps scratch out of a shared namespace.
  - **The reason is *not* that `/tmp` is outside the sandbox write scope. Corrected 2026-08-21:** `/tmp` and `/private/tmp` are both in `sandbox.filesystem.allowWrite` on every identity and in the live `settings.json` — verified with `jq -r '.sandbox.filesystem.allowWrite[]'`. This bullet asserted the opposite for weeks, which mattered because the diagnose-the-offending-segment bullet above tells you to read `settings.json` before concluding: obeying that rule discredited this one.
...
- **Do not auto-run `@nix-validator` after a Nix edit.** For a small edit, run a plain `nix eval` on the affected host attribute and report it — that is cheap and falsifiable. Reach for the full validator only at a **commit boundary**, which is already a defined stop, and say you are doing it. (Rewritten 2026-08-21. The previous version deferred large refactors to judgment at unspecified checkpoints, which no behavior could violate — running, skipping, and asking all complied. It also collided with Autonomy Boundaries below, which forbids auto-running validation *unless explicitly asked*, while this bullet obliged asking. Tying it to the commit boundary resolves both: the boundary is a real moment, and the eval report is a real artifact.)
...
**Corrected 2026-08-21: the worktrunk `post-switch` hook does not spawn a session.** This paragraph claimed it did, which made the advice read as automatic. The two hooks are `modules/home/worktrunk/hooks/tmux-switch.sh` (creates a tmux window and splits panes) and `pnpm-warm.sh` (detached install); `grep -rn claude` across that directory returns nothing. The hook name, file, and registration were all correct — only the behavior attributed to it was absent, which is why no existence check would have caught this.
