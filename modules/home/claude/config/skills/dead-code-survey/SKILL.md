...
**Core principle:** static-analysis tools _detect_; the LLM _adjudicates the residue those
tools get wrong_. Never assert something is dead from a tool result alone — tools produce
...
Set the model explicitly via the Agent tool's `model` param. Cheaper tiers _gather and
classify_; they never make the final safe/confirm call — that happens only after the Opus
adversarial pass.
...
- **Public API surface** — exports consumed _outside_ the analyzed boundary (published
...
- **Recently-added / in-flight code** — an unreferenced export in a subsystem added in the
  last few months is more likely _scaffolding not yet wired_ than rot. Static tools cannot
  ...
  the engineer**, highlighting _when_ the code was added. Give far more deference the more
...
Respect workspace boundaries — a dep used only by a _sibling_ package is not unused.
...
- **Peer-dependency chains** — a dep that a _retained_ package expects the app to provide.
  **Verify every removal candidate systematically, not ad-hoc** (this is a repeat miss):
  ...
  (a) candidate is a _regular dependency_ of a used package + the app never imports it directly
  → the app's direct declaration is a removable duplicate (e.g. a lib the used package
  re-exports); (b) candidate is a _required peer_ of a used package → keep / treat as phantom.
- **Phantom (used-but-unlisted)** — imported directly but only present transitively. This is
  a bug to _fix_ (add it), surfaced as a separate additive change — **never** a removal.
- **Runtime-only by string** — e.g. a DB driver named in an env var.
- **Dynamic-import-only** — a package pulled in _only_ via `await import("pkg")` or
...
- **Transitively-masked direct import** — a package that _is_ imported directly in source but
...
the whole repo (and any monorepo siblings) for the symbol/package name as a _string_, not
...
Write to `$ARTIFACTS/dead-code/YYYY-MM-DD-survey.md` (local dev artifact; the artifact root is
defined in the global CLAUDE.md under "Dev Artifact Storage"). Print its absolute path.
...
3. **Needs confirmation** — each item names the _trap it hit_ and the _single decision_ the
