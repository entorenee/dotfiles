...
The six formatters (`alejandra`, `yamlfmt`, `shfmt`, `stylua`, `taplo`, `prettier`) are in `modules/home/formatters` and `modules/home/yamlfmt`, imported by `roles/home/base.nix` — so conform.nvim can format on save in _any_ checkout, not just this one. They are also in the flake `devShell`, because CI has no home-manager to inherit them from. `statix` and `deadnix` are devShell-only: they are useful in this repo and nowhere else. **`actionlint` is not** — it is in the devShell for `make lint` _and_ on Mason's `ensure_installed` for nvim, because `editor.lua` lints workflow files through a path-match branch that calls `try_lint("actionlint")` directly, **bypassing `linters_by_ft` entirely**. This paragraph used to say the opposite, reasoning from `linters_by_ft` having no yaml entry; acting on that once removed `actionlint` from Mason and left every workflow file silently unlinted outside `nix develop`. Check `editor.lua`'s autocmd, not the table.

**These used to come from `mason-tool-installer`** (`modules/home/nvim/config/lua/plugins/lsp.lua`), which fetches them per-machine outside Nix, so versions drifted between hosts and neither a hook nor CI could rely on them. Mason's list has been trimmed to match — it now carries only what `roles/home/base.nix` does not put on `PATH`: `eslint_d`, `checkmake`, `actionlint`, `htmlhint`, `jsonlint`, `sqlfluff`.
...
**yamlfmt's config is the one that is not at the repo root**, and it has to be named explicitly. `modules/home/yamlfmt/config/.yamlfmt` (`retain_line_breaks`, `scan_folded_as_literal`) is deployed by home-manager to `~/.config/yamlfmt/.yamlfmt`, which is where yamlfmt finds it unaided — so conform.nvim needs no flag, but **anywhere without home-manager there is no config at all** and yamlfmt silently falls back to its defaults, stripping blank lines and unfolding folded scalars. That is not a hypothetical: it is what failed CI the first time `make check` ran on a runner, on YAML the sweep had already formatted. `make fmt`/`fmt-check` and `.githooks/pre-commit` therefore pass `-conf $(YAMLFMT_CONF)`, pointing at the module's copy rather than duplicating it at the root. A new yamlfmt call site needs the same flag.

`prettier` covers markdown only. Nearly every tracked `.json` here is written by the application that owns it — Karabiner, OrcaSlicer, Obsidian — so reformatting them would churn the next time any of those saves. The lone exception is `modules/home/nvim/config/.luarc.json`, which is hand-maintained; one file does not earn a list and a glob would sweep in the other thirty-six. `shellcheck` is not wired in yet: four of the scripts in `modules/home/bins/bin/` are zsh, which shellcheck refuses outright (`SC1071`), so it needs an exclusion mechanism and a severity floor of its own.
