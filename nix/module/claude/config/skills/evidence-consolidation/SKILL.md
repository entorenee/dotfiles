...
- **Consolidate (default)** — read the existing report artifacts and merge them. Choose this
  whenever fresh-enough reports exist under the leaves' `docs/` areas (below). No leaf re-run.
...
The leaves write to `docs/<area>/YYYY-MM-DD-slug.md` (core §7):

| Domain | Area path |
|---|---|
| Analytics friction | `docs/analytics/` |
| Error triage | `docs/error-triage/` |
| Regression analysis | `docs/regressions/` |
...
```
docs/consolidated/YYYY-MM-DD-slug.md
```
